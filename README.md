...
you can pass an option capped: true to createCollection() method
capped collection preserve the insertion order

```shell
use performance
```
```js
// below will keep only 3 documents in the collection
db.createCollection("capped", {capped: true, size: 10000, max: 3})
db.capped.insertOne({name: "Max"})
db.capped.insertOne({name: "Manu"})
db.capped.insertOne({name: "Anna"})
db.capped.insertOne({name: "Julie"})
```
...
## from mongo shell to driver

you can use shell for configure database, create collections, create indexes

driver is used for CRUD operations, aggregation, transactions, etc
